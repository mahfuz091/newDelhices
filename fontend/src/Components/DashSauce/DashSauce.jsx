...
const DashSauce = () => {
  ...
  const getSauce = async () => {
    try {
      const res = await axiosSecure.get("/api/sauce");
      setSauces(res.data);
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDelete = async (id) => {
    try {
      const response = await axiosSecure.delete(`api/sauce/${id}`);
  ...
  const columns = [
    ...
    { field: "price", headerName: "Sauce Price", flex: 1 },
    {
      ...
      renderCell: (params) => (
        <>
          <Link to={`/dashboard/sauce/update/${params.row.id}`}>
            <IconButton
              color='primary'
              aria-label='edit'
